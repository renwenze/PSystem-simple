...
class UserProxy3(BaseAgent):
    def __init__(self,topic, name,role):
        super().__init__(role) #self.llm
        self.up = UserProfile(name)
        self.round = 1
        self.history = [{"role": "system", "content": f"你是一个说服专家,你需要说服对方完全反对{topic},不接受折中方案,请根据用户的输入以对话的形式给出你的回复"}]
        self.end = False
        self.emo = -1
        print('-----用户代理初始化成功-----\n')
        time.sleep(2)
    ...
    def emo_recgonition(self,input):
        emo = self.llm(input,'''你是一个心理学家,现在你正在参加一场讨论,你的目的是说服用户反对对公众开放校园（支持对用户封闭校园），你需要记录对话者的情感进行识别，并归类到下列5种中的一种：
        *1愤怒 / 防御
        *2冷漠 / 怀疑
        *3共鸣 / 被理解
        *4矛盾 / 反思
    ...
    def get_input(self):#TODO：发起对话
        if self.round == 1:
            print('\n'+self.role+':\n')
            ask = self.llm('你是一个说服专家，你的目的是说服对方反对对公众开放校园，请你先问候对方，然后生成一段话来首先开启这个话题，简单的介绍背景并引起对话者沟通的兴趣，注意：只要按要求输出内容即可，不要有多余的输出,也不要解释',out=True)       
        else:
            ask = None
