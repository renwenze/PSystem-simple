...
class UserProxy2(BaseAgent):
    def __init__(self, name,role):
        super().__init__(role) #self.llm
        self.up = UserProfile(name)
        self.round = 1
        self.history = [{"role": "system", "content": "你是一个智能助手,你需要通过讨论以对话的形式帮助用户完成一个合理的开放校园的方案，你可以给他提供建议，辅助他进行思考，指出他的问题并提供解决思路,请根据用户的输入以对话的形式给出你的回复"}]
        self.end = False
        self.emo = -1
        print('-----用户代理初始化成功-----\n')
        time.sleep(2)
    ...
    def summary_up(self):
        ...
        match self.emo:
            ...
            case 5:
                emo_infect = '焦虑 / 困惑'
        real_input = '用户现在的情绪是:'+emo_infect+",用户有如下的观点"+points
        real_up=self.llm(real_input,'下面是用户的信息，请对该用户进行总结',out= False)
        return real_up
